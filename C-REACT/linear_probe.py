...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a C-REACT race linear probe and inspect race-associated neurons."
    )
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        ...
        default="direct",
        help="Which mention type to use for the probe dataset.",
    )
...
def load_creact_probe_data(race_groups, mention_type):
    print(f"\n--- Starting C-REACT Data Preprocessing and Filtering ({mention_type.title()} Only) ---")
    ...
    if mention_type == "direct":
        selected_patients = direct_only_patients
        filter_label = "Patients with ONLY Direct Mentions"
    else:
        selected_patients = indirect_only_patients
        filter_label = "Patients with ONLY Indirect Mentions"
    
    print(f"Total Unique Patients: {len(all_patients)}")
    print(f"{filter_label} (Filter Group): {len(selected_patients)}")

    df_filtered_patients = df_merged[
        df_merged['patient_id'].isin(selected_patients)
    ].copy()
...
def save_probe(probe, model_name, model_alias, mention_type, accuracy, label_to_group):
    ...
    probe_path = os.path.join(
        OUTPUT_DIR,
        f"race_probe_{model_alias}_{mention_type}_{timestamp}.pkl",
    )
...
def main():
    ...
    print(f"Using mention type: {mention_type}")
    
    model, tokenizer = model_setup(model_name)
    
    dataset, label_to_group = load_creact_probe_data(RACE_GROUPS, mention_type)
